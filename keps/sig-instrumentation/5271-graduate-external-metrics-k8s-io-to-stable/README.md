...
client for all the verb actions it needs to implement the `CustomMetricsClient`
interface, and second, a external metrics adapter (e.g., Prometheus Adapter) which
...
requester (owing to the aforementioned reference implementation). 
...
The `external.metrics.k8s.io` API enables operations based on metric(s) retrieved
from objects inside the cluster in a manner that is adapter-agnostic. As a part
of the aggregation layer, adapters instrumenting the API benefit from
...
- The requester, through the `external.metrics.k8s.io` client, sends out a verb
  ...
  ```
  GET /apis/external.metrics.k8s.io/v1beta1/namespaces/{ns}/{resourceType}/{resourceName}/{metricName}?{params}
  ```
  For e.g., to fetch data from all pods for a metric that match a certain
  `labelSelector`:
  ```
  GET /apis/external.metrics.k8s.io/v1beta1/namespaces/{ns}/pods/*/{metricName}?labelSelector=foo=bar
  ```
...
- Based on whether the requested metric resolves to more than one metrics in the
  matching metric family, the returned response from the adapter takes the form
  of a `k8s.io/metrics/pkg/apis/external_metrics`.MetricValue or a
    ...
    "items": [
      {
        ...
        "timestamp": "...", "windowSeconds": 0,
        "value": "300m"
      }
    ]
  }
  ```
  Note that the `windowSeconds` for a returned metric may be zero if its not a
  ranged vector (rate-based metric).
...
Yes, `v1.external.metrics.k8s.io` (w.r.t. the registered APIService object).
